...
export function index(req: Request, res: Response): void {
  ...
  res.send(
    renderPage({
      path: "/export",
      schemas: [SOFTWARE_APP_SCHEMA, EXPORT_HOWTO_SCHEMA, EXPORT_BREADCRUMB_SCHEMA],
      children: ExportPage({ tab, ...(fen !== undefined ? { fen } : {}) }),
    }),
  );
}
...
export function svg(req: Request, res: Response): void {
  ...
  const pieceStyle =
    typeof req.query["style"] === "string" ? req.query["style"].slice(0, 40) : "cburnett";
  const lightRaw = typeof req.query["light"] === "string" ? req.query["light"] : "#f0d9b5";
  const darkRaw = typeof req.query["dark"] === "string" ? req.query["dark"] : "#b58863";
