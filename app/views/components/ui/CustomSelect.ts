...
export function CustomSelect<T extends string | number>({
  ...
  const data = html`{ isOpen: false, activeIndex: -1, selectedIndex: ${selectedIndex}, toggle() {
  this.isOpen = !this.isOpen; }, open() { this.isOpen = true; }, close() { this.isOpen = false;
  this.activeIndex = -1; }, select(i) { this.selectedIndex = i; ${xModel} = ${raw(valuesJson)}[i];
  this.close(); }, scrollActive() { this.$nextTick(() => { const el =
  this.$refs.list?.querySelector( '[data-option-index="' + this.activeIndex + '"]' );
  el?.scrollIntoView({ block: 'nearest' }); }); }, keydown(e) { if (e.key === 'ArrowDown') {
  e.preventDefault(); this.open(); this.activeIndex = (this.activeIndex + 1) % ${count};
  this.scrollActive(); } else if (e.key === 'ArrowUp') { e.preventDefault(); this.open();
  this.activeIndex = (this.activeIndex - 1 + ${count}) % ${count}; this.scrollActive(); } else if
  (e.key === 'Home') { e.preventDefault(); this.open(); this.activeIndex = 0; this.scrollActive(); }
  else if (e.key === 'End') { e.preventDefault(); this.open(); this.activeIndex = ${count} - 1;
  this.scrollActive(); } else if (e.key === 'Enter' || e.key === ' ') { if (this.isOpen &&
  this.activeIndex >= 0) { e.preventDefault(); this.select(this.activeIndex); } else if (e.key ===
  'Enter') { e.preventDefault(); this.open(); } } else if (e.key === 'Tab') { this.close(); } else
  if (e.key === 'Escape') { this.close(); } } }`;

  return html`<div class="relative" @click.outside="close()" x-data="${data}">
    ${label ? html`<label id="${baseId}-label" class="select-label">${label}</label>` : ""}
    <button
      type="button"
      role="combobox"
    ...
    >
      ...
      <span class="select-icon" :class="isOpen ? 'select-icon-open' : ''" aria-hidden="true"
        >${raw(ChevronDown())}</span
      >
    </button>

    <div class="select-dropdown" data-state="closed" :data-state="isOpen ? 'open' : 'closed'">
      <div
        x-ref="list"
        id="${listboxId}"
        role="listbox"
