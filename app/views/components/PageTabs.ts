...
export function PageTabs({
  ...
  const renderGroup = (group: PageTabGroup, groupIndex: number): string => {
    ...
    if (group.label && isCollapsible) {
      heading = html` <button
        type="button"
        class="group-toggle-btn ${isExpanded
          ? 'group-toggle-active'
      ...
      >
        ...
        <span
          data-chevron-open
          class="group-chevron-open"
          ${isExpanded ? '' : 'hidden'}
          >${raw(ChevronDown('group-chevron'))}</span
        >
        <span
          data-chevron-close
          class="group-chevron-close"
          ${isExpanded ? 'hidden' : ''}
    ...
    return html` <div
      ...
      ${isCollapsible
        ? html`data-collapsible-group data-expanded="${isExpanded}"`
        : ''}
    >
      ${heading} ${itemsMarkup}
    </div>`;
