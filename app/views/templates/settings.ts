...
function getSettingsGroups(): PageTabGroup[] {
  return [
    ...
    },
    {
      label: "Access",
      items: [
        { id: "security", label: "Security", icon: ShieldCheck, authRequired: true },
    ...
    <div class="settings-identity-info">
      ...
      <div class="settings-identity-edit" data-identity-edit hidden>
        ...
        <div class="settings-identity-edit-actions">
          <button type="button" class="btn btn-primary btn-sm" data-identity-save disabled>
            <span data-identity-save-icon>${raw(Check("h-4 w-4"))}</span>
            <span data-identity-save-label>Save</span>
          </button>
          <button type="button" class="btn btn-secondary btn-sm" data-identity-cancel>
            ${raw(X("h-4 w-4"))}
          </button>
        </div>
...
function EmailCard(): string {
  return html`<section class="settings-email card-elevated" data-email-card hidden>
    <div class="settings-email-label">${raw(Mail("h-4 w-4"))} Email Address</div>
    ...
    <p class="text-xs text-text-muted">
      Changing your email sends a confirmation link to the new address. The change takes effect only
  ...
  return html`<section class="settings-details">
    ...
    <div class="settings-details-body">
      ...
      <div class="settings-info-row" data-row-created hidden>
        <span class="settings-info-row-label"> ${raw(calendarIconSvg())} Member since </span>
...
function CloudSyncCta(): string {
  return html`<section class="settings-cloud-cta" data-cloud-cta>
    <h3 class="settings-cloud-title">${raw(UserPlus("h-4 w-4"))} Cloud Synchronization</h3>
    <p class="settings-cloud-text">
      Create a ChessViewer account to sync your boards, history, and custom themes across all your
      devices. Your current local data will be automatically migrated to the cloud.
    </p>
...
function AccountActions(): string {
  return html`<section class="settings-danger-section" data-account-actions hidden>
    <h3 class="settings-danger-title">${raw(ShieldAlert("h-4 w-4"))} Manage Account</h3>
    <p class="settings-danger-text">
      Permanently erase your account and all associated cloud data. This action is irreversible. If
      you prefer support-handled deletion, email us with your
...
function AccountSection(): string {
  return html`<div class="settings-page-space">
    ...
    ${IdentityHeader()} ${EmailCard()} ${AccountDetails()} ${CloudSyncCta()} ${AccountActions()}
    ${Modal({
        ...
        <div class="space-y-4">
          ...
          <div class="space-y-3">
            ...
            <div data-delete-mfa-wrap>
              <label class="form-label" for="del-mfa">2FA Verification Code</label>
              <input
                id="del-mfa"
                type="text"
            ...
            <p class="text-xs font-semibold text-error" data-delete-error hidden></p>
            <p class="text-center text-xs text-text-muted" data-delete-progress hidden>
              Erase in progress...
            </p>
          </div>
        </div>
...
function AppearanceSection(): string {
  return html`<div class="settings-page-space">
    ...
    ${SettingsBlock({
      title: "Theme",
      description: "Choose between light and dark themes, or follow your system setting.",
      children: html`
        <div class="settings-theme-mode">
          ...
          <div
            ...
            data-theme-mode
          >
            ${themeOption("light", "Light", Sun)} ${themeOption("dark", "Dark", Moon, true)}
    ...
    ${SettingsBlock({
      ...
      children: html`
        <div class="settings-contrast-grid" role="radiogroup" aria-label="Contrast" data-contrast>
          ${contrastOption(
            "normal",
            "Default",
...
function themeOption(value: string, label: string, icon: typeof Sun, active = false): string {
  return html`<button
    type="button"
    role="radio"
    aria-checked="${active}"
    data-theme-option="${value}"
    class="settings-theme-option ${active ? "settings-theme-option-active" : ""}"
  >
    ${raw(icon("settings-theme-option-icon"))} ${label}
  </button>`;
}

function contrastOption(value: string, label: string, description: string, active = false): string {
  return html`<button
    type="button"
    role="radio"
    aria-checked="${active}"
      ...
      children: html`
        <div class="space-y-3">
          ...
          <p class="settings-sim-banner" data-motion-banner hidden>
            ...
            <span
              >Animations across the app are replaced with instant transitions, regardless of your
              system setting.</span
            >
          </p>
          <div class="settings-os-panel" data-os-panel hidden>
            <p class="mb-2">
              This follows your operating system&apos;s motion setting. To change it, open:
            </p>
            <ul class="space-y-1">
              <li>
                <span class="settings-os-label">GNOME:</span> Settings → Accessibility → Seeing →
                Reduce Animation
              </li>
              <li>
                <span class="settings-os-label">macOS:</span> System Settings → Accessibility →
                Display → Reduce Motion
              </li>
              <li>
                <span class="settings-os-label">Windows:</span> Settings → Accessibility → Visual
...
function SecurityOverview(): string {
  return html`<div class="settings-overview-grid">
    ...
    <div class="settings-overview-card">
      <span class="settings-overview-icon"> ${raw(Clock("settings-overview-icon-svg"))} </span>
      <div>
  ...
  return html`<div class="settings-mfa" data-mfa-panel>
    ...
    <div class="settings-mfa-setup" data-mfa-setup hidden>
      ...
      <p class="text-xs text-text-secondary">
        Use a time-based one-time passcode (TOTP) from an authenticator app as a second factor when
        you sign in.
      </p>
    </div>
    <div class="settings-mfa-enroll" data-mfa-enroll hidden>
      <p class="text-sm font-semibold text-text-primary" data-mfa-enroll-step></p>
      ...
      <div class="settings-mfa-secret" data-mfa-secret hidden>
        ...
        <button type="button" class="btn-icon-sm" data-mfa-secret-copy aria-label="Copy secret">
          ${raw(copySvg())}
        </button>
      </div>
      ...
      <div class="settings-mfa-backup" data-mfa-backup hidden>
        <p class="text-xs font-semibold text-text-secondary">Backup codes — save them now:</p>
        <div class="settings-mfa-backup-codes" data-mfa-backup-codes></div>
        <div class="settings-mfa-backup-actions">
          <button type="button" class="btn btn-secondary btn-sm" data-mfa-copy-codes>
            ${raw(copySvg())} Copy codes
          </button>
          <button type="button" class="btn btn-secondary btn-sm" data-mfa-download-codes>
            ${raw(Download("h-4 w-4"))} Download
          </button>
          <button type="button" class="btn btn-primary btn-sm" data-mfa-done>Done</button>
        </div>
      </div>
    ...
    <div class="settings-mfa-enabled" data-mfa-enabled hidden>
      <div class="settings-mfa-enabled-head">
        ...
        <div>
          <p class="text-sm font-semibold text-text-primary">Two-Factor Authentication is on</p>
          <p class="text-xs text-text-secondary">An authenticator app is required to sign in.</p>
        </div>
      </div>
      <button type="button" class="btn btn-danger btn-sm" data-mfa-disable>Disable 2FA</button>
    </div>
  </div>`;
}
...
function ChangePasswordCard(): string {
  return html`<div class="settings-password-card">
    <h3 class="settings-card-title">${raw(KeyRound("h-4 w-4"))} Change Password</h3>
    <p class="text-xs text-text-secondary mb-3">
      Set a new password (at least 8 characters). It applies immediately on this device.
    </p>
    <div class="flex flex-col gap-2 settings-max-w-md">
      ...
      />
      <p class="text-xs text-error" data-password-error hidden>Passwords do not match.</p>
      <div class="flex flex-wrap items-center gap-2 pt-1">
        <button type="button" class="btn btn-primary" data-change-password disabled>
          <span data-change-password-label>Update Password</span>
        </button>
        <button type="button" class="btn btn-secondary" data-email-reset disabled>
...
function SecuritySection(): string {
  return html`<div class="settings-page-space">
    ${SettingsHeading({
      icon: ShieldCheck,
      title: "Security",
      description: "Protect your account with two-factor authentication and control your sessions.",
    })}

    <div data-security-content>
      <div class="settings-signed-out-panel" data-security-signed-out>
        <p class="font-semibold text-text-primary">Sign in to manage security</p>
        <p class="mx-auto mt-2 text-sm text-text-secondary">
          Two-factor authentication and password management are available once you&apos;re signed
          in.
        </p>
      </div>
      ...
      <div class="settings-signed-in" data-security-signed-in hidden>
        ...
        <section class="settings-card-stack">
          <div class="settings-card-stack-block">
            ...
            <p class="text-xs text-text-secondary mb-3">
              Use a time-based one-time passcode (TOTP) from an authenticator app as a second factor
              when you sign in.
            </p>
            ${TwoFactorPanel()}
          </div>
        ...
        <section class="card-elevated">
          <h3 class="settings-card-title">${raw(LogOut("h-4 w-4"))} Active Sessions</h3>
          <p class="text-xs text-text-secondary mb-3">
            Sign out of every device and browser at once. Use this if you suspect your account is
            signed in somewhere you don&apos;t recognise.
          </p>
          <button type="button" class="settings-danger-outline" data-signout-everywhere>
            <span data-signout-everywhere-label>Sign out everywhere</span>
          </button>
        </section>

        <section class="settings-activity card-elevated">
          <h3 class="settings-card-title">${raw(History("h-4 w-4"))} Security Activity</h3>
  ...
  return html`<div class="settings-page-space">
    ...
    <section class="card-elevated">
      ...
      <ul class="settings-storage-list">
        ${storageCategories
          .map(
            (cat) => html`<li class="settings-storage-row">
              <span class="text-sm text-text-primary">${cat.label}</span>
              <span class="settings-storage-right">
                <span class="text-xs font-medium text-text-secondary" data-storage-bytes="${cat.id}"
                  >0 B</span
                >
                <button
                  type="button"
    ...
    <input type="file" accept="application/json,.json" class="hidden" data-import-file />

    <div class="settings-message" data-settings-message hidden>
      <span class="settings-message-icon">${raw(Check("settings-message-icon-svg"))}</span>
...
export function SettingsPage(options: SettingsPageOptions = {}): string {
  ...
  const activeTab = requestedTab && VALID_TAB_IDS.has(requestedTab) ? requestedTab : "profile";
