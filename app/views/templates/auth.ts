...
function PasswordField({
  ...
  return html`<div class="auth-password-wrap">
    ...
    >
      ...
      <span data-toggle-icon="hide" hidden
        >${raw(EyeOff('auth-password-icon'))}</span
      >
    </button>
  </div>`;
}
  ...
  return AuthShell({
    ...
    children: html`
      ...
      <p class="auth-bottom-text">
        New here?
        ${AuthLink({ href: '/auth/sign-up', children: 'Create an account' })}
      </p>
    `
  });
}
    ...
    children: html`
      ...
      <div class="auth-success-card" data-signup-success hidden>
        <div class="auth-success-icon">
          ${raw(MailCheck('auth-success-icon-svg'))}
        </div>
      ...
      >
        ...
        <div class="auth-field-group">
          <label for="signup-confirm" class="auth-label"
      ...
      <p class="auth-bottom-text">
        ...
        ${AuthLink({ href: '/auth/sign-in', children: 'Sign in' })}
      </p>
    `
  });
}
    ...
    children: html`
      <div class="auth-mfa-head">
        <div class="auth-mfa-icon">
          <span data-mfa-icon="totp"
            >${raw(ShieldAlert('auth-mfa-icon-svg'))}</span
          >
          <span data-mfa-icon="backup" hidden
            >${raw(KeyRound('auth-mfa-icon-svg'))}</span
          >
        </div>
      ...
      <div
        class="auth-alert auth-alert-center"
        data-form-error
        role="alert"
        hidden
      ></div>
      ...
      <div class="auth-mfa-footer">
        <button
          type="button"
          class="auth-link auth-mode-toggle"
          data-mfa-toggle
        >
...
export function AuthSuccessNote(): string {
  return html`<div class="auth-note">
    <span class="auth-note-icon"
      >${raw(CheckCircle('auth-note-icon-svg'))}</span
    >
