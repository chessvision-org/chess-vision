...
function ChangelogEntryRow(entry: {
  ...
  return html`<li class="changelog-entry">
    ...
    <span class="changelog-entry-text">
      ${entry.scope
        ? html`<span class="changelog-scope">${entry.scope}:</span>`
        : ''}
...
function ChangelogSection(year?: string): string {
  ...
  const totalEntries = years.reduce(
    (n, y) =>
      n +
      y.months.reduce(
        (mn, m) => mn + m.groups.reduce((gn, g) => gn + g.entries.length, 0),
        0
      ),
    0
  );
  ...
  const currentYear = year
    ? years.find((y) => y.year === Number(year))
    : years[0];
  ...
  const yearLinks = (y: ChangelogYear): string => {
    ...
    return html`<a
      ...
      class="changelog-year-btn ${isActive
        ? 'changelog-year-active'
        : 'changelog-year-inactive'}"
      >${y.year}</a
    >`;
  };
  ...
  const newerLink = (index: number): string => {
    if (index === 0) {
      return html`<span class="changelog-nav-btn" aria-disabled="true"
        >Newer</span
      >`;
    }
    return html`<a
      href="?tab=changelog&year=${years[index - 1].year}"
      class="changelog-nav-btn"
      >Newer</a
    >`;
  };
              ...
              .map(
                (month) =>
                  html`<li class="mb-8 list-none">
                    ...
                    ${month.note
                      ? html`<p
                          class="mb-4 text-base leading-relaxed text-text-secondary"
                        >
...
function ThanksSection(): string {
  ...
  return html` <div class="about-section">
    ...
    <div class="space-y-5 text-base leading-relaxed text-text-secondary">
      <p>
        The interface is a ${ext('https://preactjs.com', 'Preact')} app written
        in ${ext('https://www.typescriptlang.org', 'TypeScript')}, built with
        ${ext('https://vitejs.dev', 'Vite')}, and styled with
        ${ext('https://tailwindcss.com', 'Tailwind CSS')}. Fast navigation
        between pages is handled by
        ${ext('https://github.com/molefrog/wouter', 'wouter')}, and the page
        transitions are hand-written in plain CSS. The board&apos;s
      ...
      <p>
        ...
        ${ext('https://lucide.dev', 'Lucide')}, and the QR codes for Two-Factor
        Authentication (2FA) are rendered with
        ${ext('https://github.com/nuintun/uqr', 'uqr')}. All account handling,
        auth, and cloud sync are entrusted to
        ${ext('https://supabase.com', 'Supabase')}.
      </p>
      <p>
        The app is packaged with ${ext('https://www.docker.com', 'Docker')} and
        served by ${ext('https://nginx.org', 'nginx')}. The position-database
        search would not be possible without the open chess data published by
        ${ext('https://lichess.org', 'Lichess')} and
        ${ext('https://chessdb.cn/queryc_en/', 'ChessDB')}, together with the
        problem collections at
      ...
      <p>
        ...
        ${ext('https://github.com/iccccccccccccc', 'iccccccccccccc')}, whose
        efforts have directly improved the codebase and the community.
      </p>

      <p>
        Finally, thank you to everyone who reports bugs, suggests improvements,
        contributes code, and simply uses ChessViewer. The project is better
...
export function AboutPage(activeTab: string, year?: string): string {
  ...
  return html` <div class="min-h-full bg-bg">
