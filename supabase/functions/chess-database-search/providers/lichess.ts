...
export async function searchLichess(fen: string): Promise<SearchResponse> {
  ...
  try {
    const mText = await fetchText(`https://explorer.lichess.org/masters?${qs}`, { headers });
    if (mText !== null && hasGames(mText)) return { found: true, database: "LICHESS", url };

    const oText = await fetchText(`https://explorer.lichess.org/lichess?${qs}`, { headers });
