...
Deno.serve(async (req: Request) => {
  ...
  // Cache check
  const cachedPair = noCache ? null : await readCache(supabase, board, isProviderPair);
  const cachedLichess = noCache
    ? null
    : ((await readCache(supabase, lichessCacheKey(fen), isLichessHit))?.lichess ?? null);
  ...
  if (!cachedLichess) {
    await writeCache(supabase, lichessCacheKey(fen), { lichess: map.lichess }, map.lichess.found);
  }
